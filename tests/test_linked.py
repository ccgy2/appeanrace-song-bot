...
FRONT=BACK
FIREBASE_FALLBACK='https://appearance-song.web.app'
...
class LinkedConfigurationTests(unittest.TestCase):
    def test_preconfigured_domains(self):
        ...
        self.assertEqual(d['web_origins'], (BACK, FIREBASE_FALLBACK, ALIAS))
    ...
    def test_legacy_wrong_url_variables_cannot_break_this_bundle(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(os.environ, {
            ...
            self.assertEqual(s.web_origins, (BACK, FIREBASE_FALLBACK, ALIAS))
    ...
    def test_sqlite_is_default_even_when_old_firebase_secret_remains(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(os.environ, {
            'WEB_ONLY':'true','DATA_DIR':tmp,'WEB_ADMIN_PASSWORD':PASSWORD,
            'FIREBASE_SERVICE_ACCOUNT':'old-secret-that-must-not-be-used',
        }, clear=True):
            s=Settings.from_env()
            self.assertEqual(s.storage_backend, 'sqlite')
            self.assertEqual(s.firebase_key, '')
            store=Store(s)
            try:
                self.assertEqual(store.mode, 'local')
            finally:
                store.sql.close()
...
class LinkedAPIRegressionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_alias_login(self):
        ...
        await self.login(FIREBASE_FALLBACK)
    ...
    async def test_untrusted_domains_rejected_and_same_origin_token_allowed(self):
